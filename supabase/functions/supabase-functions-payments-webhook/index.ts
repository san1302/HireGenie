...
async function handleSubscriptionCreated(supabaseClient, body) {
  ...
  try {
    ...
    const { data, error } = await supabaseClient.from("subscriptions").insert({
      ...
      ended_at: body.data.ended_at ? new Date(body.data.ended_at).getTime() : null,
      canceled_at: body.data.canceled_at ? new Date(body.data.canceled_at).getTime() : null,
...
async function handleSubscriptionUpdated(supabaseClient, body) {
  try {
    const { data: existingSub } = await supabaseClient.from("subscriptions").select().eq("polar_id", body.data.id).single();
    if (existingSub) {
...
async function handleSubscriptionActive(supabaseClient, body) {
  try {
    const { data: activeSub } = await supabaseClient.from("subscriptions").select().eq("polar_id", body.data.id).single();
    if (activeSub) {
      const { error } = await supabaseClient.from("subscriptions").update({
        status: body.data.status,
        started_at: new Date(body.data.started_at).getTime()
...
async function handleSubscriptionCanceled(supabaseClient, body) {
  try {
    const { data: canceledSub } = await supabaseClient.from("subscriptions").select().eq("polar_id", body.data.id).single();
    if (canceledSub) {
      const { error } = await supabaseClient.from("subscriptions").update({
        status: body.data.status,
        canceled_at: body.data.canceled_at ? new Date(body.data.canceled_at).getTime() : null,
...
async function handleSubscriptionUncanceled(supabaseClient, body) {
  try {
    const { data: uncanceledSub } = await supabaseClient.from("subscriptions").select().eq("polar_id", body.data.id).single();
    if (uncanceledSub) {
      const { error } = await supabaseClient.from("subscriptions").update({
        status: body.data.status,
...
async function handleSubscriptionRevoked(supabaseClient, body) {
  try {
    const { data: revokedSub } = await supabaseClient.from("subscriptions").select().eq("polar_id", body.data.id).single();
    if (revokedSub) {
      const { error } = await supabaseClient.from("subscriptions").update({
        status: 'revoked',
        ended_at: body.data.ended_at ? new Date(body.data.ended_at).getTime() : null
...
async function handleOrderCreated(supabaseClient, body) {
  return new Response(JSON.stringify({
